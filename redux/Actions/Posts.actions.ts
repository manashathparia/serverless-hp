...
export const getAllPostsAction = (_posts) => {
...
export const getWpPostAction = (_post) => {
    const post = {
        ..._post,
...
export const updateTotalPageAction = (totalPosts, totalPages) => ({
...
export const getWpAllPosts = (pageNo) => async (dispatch) => {
    try {
...
export const getWpPost = (slug) => async (dispatch, getState) => {
    try {
        const {
...
export const setDisplayPosition = (x, y) => ({
    type: UPDATE_DISPLAY_POSITION,
    payload: [x, y],
});

export const updateFetchedPosts = (post) => async (dispatch, getState) => {
    const {
        posts: { fetched_posts: fetchedPosts },
    } = await getState();
    dispatch({
